...
def single_model_run_SEIR(t_final,total_pop,n,p_edge,par):
    ...
    for i in range(n):
        ...
        while n_edges_per_node[i] > cdf[c_edges]:
            c_edges += 1
        n_edges_per_node[i] = c_edges + min_edges
        list_of_stubs.extend([i]*(c_edges+min_edges))
    ...
    while len(list_of_stubs)>0:
        if (len(list_of_stubs) == 2) & (list_of_stubs[0] == list_of_stubs[1]): # cannot connect to own node
            ...
            break
        edge = random.sample(list_of_stubs,2)
        if edge[0] != edge[1]:
            edges_mat[edge[0]][edge[1]] = 1
            edges_mat[edge[1]][edge[0]] = 1
            list_of_stubs.remove(edge[0])
